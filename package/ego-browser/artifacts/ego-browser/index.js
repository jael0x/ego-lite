...
function resolveParam(node) {
    ...
    if (node.type === "ObjectPattern") {
        const props = (node.properties || []).map((p) => p.key?.name || "?").join(", ");
...
async function cdp(method, params = {}, sessionId = undefined) {
    ...
    const response = await send$1({ method, params, session_id: sessionId });
...
async function gotoAndWait(url, options = {}) {
    const navigation = await gotoUrl$1(url);
...
async function elementEval(selectorOrRef, fn, ...args) {
    await ensureRefMapForRef(selectorOrRef);
...
async function siteSkillsForUrl$1(url, options = {}) {
    const hostname = urlHostname$1(url);
    if (!hostname) {
        return [];
    }
...
async function switchTaskSpaceIfNamed(ego, name) {
    if (name === undefined)
        return;
    const spaces = await listTaskSpaces();
 ...
 * @returns {Promise<void>}
 */
async function completeTaskSpace(name, options) {
 ...
 * @returns {Promise<void>}
 */
async function handOffTaskSpace(name) {
    const ego = globalThis.ego;
 ...
 * @returns {Promise<void>}
 */
async function takeOverTaskSpace(name) {
    const ego = globalThis.ego;
...
function isUserControlError(err) {
 ...
 * @returns {Promise<void>}
 */
async function waitForAgentControl(name, options = {}) {
    if (typeof name !== "string" || !name) {
...
async function gotoUrl(url) {
    const result = await gotoUrl$1(url);
...
var helpers = /*#__PURE__*/Object.freeze({
    ...
    cdp: cdp,
    click: click,
    completeTaskSpace: completeTaskSpace,
    currentTab: currentTab,
    dispatchKey: dispatchKey,
    doubleClick: doubleClick,
    dragMouse: dragMouse,
    ...
    gotoAndWait: gotoAndWait,
    gotoUrl: gotoUrl,
    handOffTaskSpace: handOffTaskSpace,
    ...
    listTabs: listTabs,
    listTaskSpaces: listTaskSpaces,
    loadAgentHelpers: loadAgentHelpers,
    newTab: newTab,
    openOrReuseTab: openOrReuseTab,
    pageInfo: pageInfo,
    pressKey: pressKey,
    runSiteBrowserTool: runSiteBrowserTool,
    runSiteTool: runSiteTool,
    scroll: scroll,
    scrollBy: scrollBy,
    scrollToBottomUntil: scrollToBottomUntil,
    siteSkills: siteSkills,
    siteSkillsForUrl: siteSkillsForUrl,
    snapshot: snapshot,
    snapshotRaw: snapshotRaw,
    snapshotText: snapshotText,
    switchTab: switchTab,
    takeOverTaskSpace: takeOverTaskSpace,
    typeText: typeText,
    uploadFile: uploadFile,
    useOrCreateTaskSpace: useOrCreateTaskSpace,
    wait: wait,
...
async function runMain(options = {}) {
    ...
    const code = options.stdinText !== undefined ? options.stdinText : await readAll(options.stdin || stdin);
    if (!code.trim()) {
        write(stderr$1, USAGE);
        return 2;
    }
...
async function executionContext(stdout$1 = stdout) {
    const agentHelpers = await loadAgentHelpers();
    const context = publicContext({ ...helpers, ...agentHelpers });
    context.cliLog = (...args) => {
        write(stdout$1, `${args.map(formatCliLogValue).join(" ")}\n`);
    };
    return context;
}
function publicContext(moduleLike) {
    const out = {};
...
function installEgoSdk(target = globalThis, options = {}) {
    ...
    for (const [name, value] of Object.entries(context)) {
        ...
        const exposed = SYNC_HELPERS.has(name) ? value : async (...args) => {
            await readySignal;
            if (readyError) {
                throw readyError;
            }
...
function createCliLog(stream = process.stdout) {
    return (...args) => {
...
export { INTERNAL_URL_PREFIXES, NAME, __testing, captureScreenshot, cdp, click, completeTaskSpace, currentTab, dispatchKey, doubleClick, dragMouse, drainEvents, elementCenter, elementEval, ensureRealTab, fillInput, gotoAndWait, gotoUrl, handOffTaskSpace, helperContext, hover, httpGet, iframeTarget, installEgoSdk, js, learnContext, listTabs, listTaskSpaces, loadAgentHelpers, newTab, openOrReuseTab, pageInfo, pressKey, runMain, runSiteBrowserTool, runSiteTool, scroll, scrollBy, scrollToBottomUntil, siteSkills, siteSkillsForUrl, snapshot, snapshotRaw, snapshotText, switchTab, takeOverTaskSpace, typeText, uploadFile, useOrCreateTaskSpace, wait, waitForAgentControl, waitForElement, waitForLoad, waitForNetworkIdle };
